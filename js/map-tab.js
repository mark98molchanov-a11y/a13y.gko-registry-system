(function clearNSPDStorage() {
    console.log('🧹 Очистка кеша НСПД из localStorage...');
    
    // 1. Удаляем все ключи связанные с НСПД
    const keysToRemove = [
        'nspd_data',
        'deals_csv_gist_url',
        'deals_csv_gist_id'
    ...
    // 2. Очищаем глобальные переменные (если они уже существуют)
    if (typeof window.allDealsFlat !== 'undefined' && window.allDealsFlat) {
        let clearedCount = 0;
        window.allDealsFlat.forEach(deal => {
            if (deal.cad_nspd) {
                deal.cad_nspd = null;
                clearedCount++;
            }
        });
        console.log(`🧹 Очищено ${clearedCount} номеров НСПД в allDealsFlat`);
    }
    
    if (typeof window.dealsData !== 'undefined' && window.dealsData) {
        let clearedCount = 0;
        Object.keys(window.dealsData).forEach(key => {
            const deals = window.dealsData[key];
        ...
        console.log(`🧹 Очищено ${clearedCount} номеров НСПД в dealsData`);
    }
    
    // 3. Сбрасываем флаги
    if (typeof window._isNSPDSearch !== 'undefined') {
        window._isNSPDSearch = false;
    }
    if (typeof window._isSyncRunning !== 'undefined') {
        window._isSyncRunning = false;
    }
    
    console.log('✅ Кеш НСПД полностью очищен при загрузке страницы');
    console.log('📌 Все номера НСПД будут загружены заново из Gist');
})();
...
async function searchNSPD(quarter, targetArea, targetType, locationKeywords = [], tolerance = 1, signal = null) {
    ...
    try {
        ...
        const hasStreet = normalizedDealStreet && normalizedDealStreet !== '' && normalizedDealStreet !== 'nan';
        ...
        for (const f of features) {
            ...
            if (hasStreet && nspdStreet) {
                ...
                // Дополнительно: проверяем, содержит ли одна улица другую
                if (!streetMatch) {
                    streetMatch = normalizedDealStreet.includes(nspdStreet) || nspdStreet.includes(normalizedDealStreet);
                }
            }
