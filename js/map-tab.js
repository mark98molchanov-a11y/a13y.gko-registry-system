  ...
  if (wrapperQuarters.length > 0) {
    window.wrapperLayer = L.geoJSON(wrapperQuarters, {
        ...
        onEachFeature: function(feature, layer) {
            ...
            // ✅ ИСПОЛЬЗУЕМ ПОПАП (у него есть крестик)
            layer.bindPopup(getPopupContent(), {
                className: 'custom-popup',
                maxWidth: 300,
                closeButton: true  // ← КРЕСТИК
            });
            ...
            // ✅ ОБРАБОТЧИК ЗАКРЫТИЯ ПОПАПА (крестик) — как у обычных кварталов
            layer.on('popupclose', function(e) {
                if (currentLevel === 2) {
                    console.log('Попап обертки закрыт → обновление таблицы');
                    window.selectedQuarterCadNumber = null;
                    setTimeout(() => {
                        if (currentLevel === 2) {
                            onPopupClose('quarter', null);
                        }
                    }, 300);
                }
            });
...
function onPopupClose(levelName, districtId) {
    ...
    if (levelName === 'quarter') {
        console.log('🔄 Закрытие попапа/тултипа → обновление таблицы');
