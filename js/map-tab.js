 ...
 if (wrapperQuarters.length > 0) {
    window.wrapperLayer = L.geoJSON(wrapperQuarters, {
        ...
        onEachFeature: function(feature, layer) {
            ...
            // ✅ ФУНКЦИЯ ДЛЯ ПОСТРОЕНИЯ ПОПАПА (как у обычных кварталов)
            ...
            // ✅ ПРИВЯЗЫВАЕМ ПОПАП (как у обычных кварталов)
            ...
            layer.on('click', function(e) {
                // Обновляем попап перед открытием
            ...
            // ✅ ХОВЕР - просто подсветка, без тултипа
            ...
            layer.on('popupclose', function(e) {
                // Сбрасываем выбранный квартал, если это была обертка
                if (window.selectedQuarterCadNumber === cadNum) {
                    // Проверяем, не был ли это клик по обертке
                    setTimeout(() => {
                        // Если попап закрыт, сбрасываем выбор
                        window.selectedQuarterCadNumber = null;
                        renderDealsTable();
...
async function updateGitHubCSVWithNSPD(token) {
    ...
    try {
        // ✅ 1. ПОЛУЧАЕМ ID РЕЛИЗА (через прокси)
        console.log('📥 Получение информации о релизе...');
        const releaseResponse = await fetch(`${proxyUrl}/api/github?endpoint=releases/tags/${releaseTag}`, {
        ...
        // ✅ 4. УДАЛЯЕМ СТАРЫЙ ASSET (через прокси)
        if (assetId && assetUrl) {
            console.log('🗑️ Удаление старого asset...');
            const deleteResponse = await fetch(`${proxyUrl}/api/github?endpoint=releases/assets/${assetId}`, {
                method: 'DELETE',
                headers: { 'Authorization': `token ${token}` }
            });
        ...
        // ✅ 5. ЗАГРУЖАЕМ НОВЫЙ ASSET (через прокси)
        console.log('📤 Загрузка нового CSV в релиз...');
        const uploadResponse = await fetch(`${proxyUrl}/api/github/upload`, {
            method: 'POST',
            headers: {
