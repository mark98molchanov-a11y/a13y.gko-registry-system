...
class MarketValuationApp {
                ...
                <div class="grid grid-cols-1 lg:grid-cols-2 gap-6">
                    <div class="bg-white rounded-xl border border-slate-200 shadow-sm">
                        ...
                        <form id="valuationForm" class="p-5 space-y-4">
                            ...
                            <div>
                                <label class="block text-sm font-medium text-slate-700 mb-1">Кадастровая стоимость, ₽/м²</label>
                                <input type="number" id="cadastralPrice" step="0.01" min="0" class="w-full px-3 py-2 border border-slate-300 rounded-lg" placeholder="Например: 120000">
                                <p class="text-xs text-slate-400 mt-1">Если не заполнено — будет использована медиана по городу + каскад</p>
                            </div>
    ...
    downloadTemplate() {
        ...
        const template = [
            {'Тип объекта':'Помещение','Площадь (м²)':60,'Город (МО)':'Ноябрьск','Материал стен':'Кирпич','Наименование':'Квартира','Назначение':'Жилое','Год постройки':2015,'ВРИ (для земли)':'','Категория земель':'','Кадастровый номер':'','КС (₽/м²)':''},
            {'Тип объекта':'Здание','Площадь (м²)':100,'Город (МО)':'г. Салехард','Материал стен':'Монолит','Наименование':'Магазин','Назначение':'Торговое','Год постройки':2020,'ВРИ (для земли)':'','Категория земель':'','Кадастровый номер':'','КС (₽/м²)':''},
            {'Тип объекта':'Земельный участок','Площадь (м²)':600,'Город (МО)':'Ноябрьск','Материал стен':'','Наименование':'','Назначение':'','Год постройки':2024,'ВРИ (для земли)':'для ИЖС','Категория земель':'Земли населенных пунктов','Кадастровый номер':'','КС (₽/м²)':''},
            {'Тип объекта':'Земельный участок','Площадь (м²)':1000,'Город (МО)':'Надым','Материал стен':'','Наименование':'','Назначение':'','Год постройки':2024,'ВРИ (для земли)':'для садоводства','Категория земель':'Земли сельхозназначения','Кадастровый номер':'','КС (₽/м²)':''}
        ];
        if (XLSX) {
    ...
    mapColumns(row) {
        return {
            ...
            kadastr_price: parseFloat(row['КС (₽/м²)']||row['ks']||0) || '',
                    ...
                    try {
                        ...
                        if (resp.ok) {
                            ...
                            results.push({
                                ...
                                'КС введена (₽/м²)':row.kadastr_price || '—',
